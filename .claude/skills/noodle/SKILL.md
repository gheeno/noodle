...
pattern table (no step code); locators: accessible-name → POM yaml →
...
in the URL — give shared files `match: {}` or their keys never resolve. Wrong element? `noodle inspect <url> "<phrase>"` (`inspect_locator`).

## Workspace

`noodle init <ws>` scaffolds; each app self-contained:
`features/`, `resources/` (env yaml, gitignored `<app>_secrets.env`,
`pageobjects/*_pom.yaml`), `report/`.
