...
def author_test(app_name: str | None = None, base_url: str | None = None,
    ...
    return core.collapse_green(core.author_test(
        ...
        run_scope=run_scope, auto_fix=auto_fix, headless=headless,
        retries=retries, serve_reports=serve_reports,
        workspace=ws, door="mcp"), workspace=ws)
...
# NOOD_0210 — the implementation lives in noodle/docs_reader.py (pure stdlib)
# so `noodle docs` reaches it without importing this module and its FastMCP
# instance. Re-exported here as the MCP tool.
@_tool()
def read_docs(name: str | None = None, query: str | None = None,
    ...
    from noodle.docs_reader import read_docs as _read_docs
    return _read_docs(name=name, query=query, section=section)
