...
| `--brief` | off | Print each step TEMPLATE once with its names underneath, instead of a full sentence per control (compact mode) | NOOD_0179: a 40-step list re-sends the same three sentences 40 times. NOOD_0231 — the flag existed and appeared on no agent surface, so a reviewed session paid ~1,400 tokens for output this collapses; it is on the skill cards now. |
| `--delta` | off | With a chained `--do`: print only the NEWEST reveal stage in full; the landing page and each earlier stage collapse to one `stage N '<label>': M controls` line (JSON: `delta_skipped`) | NOOD_0231: walking a 7-verb chain takes one probe per verb — the next control's name is unknowable until the prior one executes — and each probe reprinted every stage before it, O(n²) output for O(n) new facts. Opt-in, because output that has fallen out of context cannot be recovered from here; re-run without it for everything. A no-op when there are no reveal stages, so a single-shot probe is never summarised away, and verdict keys (`author_ready`, `do_failed`, warnings) are never collapsed. |
